...
class Str	// Str 클래스를 선언
{
	...
	public:
		...
		Str(const char *neyong); // neyong은 초기화할 내용이 들어간다.
		~Str(); // Str 객체의 소멸자 
		...
		int compare(const char *a);  // a의 내용과 string에 strcmp 함수를 사용한 결과를 리턴한다.
		void operator=(const char *a); // a 포인터가 지시하는 문자열을 string에 대입한다
